...
```{r Alpha and Jitter} ggplot(aes(x = age, y = friend_count), data = pfd) + 
geom_jitter(position=position_jitter(h=0), alpha=1/20) + xlim(c(13,90)) + 
coord_trans(y="sqrt") ```

*** ```{r Friends_initiated versus Age} ggplot(aes(x= age,
y=friendships_initiated), data=pfd) + geom_point(alpha=1/20,
position=position_jitter(h=0)) + xlim(c(13,90)) + coord_trans(y='sqrt')
